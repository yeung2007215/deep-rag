...
def rerank_docs(query: str, docs, top_k: int = 5):
    """
    docs: List[Document] (LangChain Document)
    return: reranked_docs, scored_pairs
    """
    if not docs:
        return [], []

    pairs = [(query, d.page_content) for d in docs]
    scores = reranker.predict(pairs)  # ndarray of float
    ranked = sorted(zip(docs, scores), key=lambda x: x[1], reverse=True)

    reranked_docs = [d for d, s in ranked[:top_k]]
    scored_pairs = [(s, d) for d, s in ranked[:top_k]]
    return reranked_docs, scored_pairs
